...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contextText) {
      // 자료를 근거로 답할 때의 규칙을 함께 붙인다.
      //
      // 프리셋 프롬프트에 각각 적지 않는 이유: 이 실패는 특정 비서의 문제가
      // 아니라 문서를 근거로 답하는 모든 경로에서 생긴다. 직원이 만든 개인
      // 비서에도 똑같이 필요하고, 프리셋 19개에 흩어 두면 한쪽만 고치게 된다.
      //
      // 실제로 겪은 것: 단위가 없는 물량 표를 읽고 "2,000톤"이라고 답했다.
      // 숫자는 맞았지만 '톤'은 문서에 없는 값이다. 공공기관에서는 그 답이
      // 보고서로 들어가고 나중에 근거를 찾을 수 없게 된다.
      fullSystemPrompt = [
        systemMessage,
        '',
        '참고 자료:',
        ...
        '',
        '[자료 인용 규칙]',
        '- 수치·날짜·금액·비율은 자료에 적힌 그대로 옮기세요. 반올림하거나 환산하지 마세요.',
        '- 자료에 없는 단위(톤·원·명 등)를 임의로 붙이지 마세요. 단위가 없으면 없는 대로 두고,',
        '  필요하면 "자료에 단위 표기가 없습니다"라고 밝히세요.',
        '- 표의 값을 답할 때는 어느 행·열에서 가져왔는지 알 수 있게 항목명을 함께 쓰세요.',
        '- 자료에서 확인되지 않는 것은 추론해 채우지 말고 확인되지 않는다고 말하세요.',
      ].join('\n');
